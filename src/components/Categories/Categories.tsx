...
class Categories extends React.Component<any, any> {
    ...
    render() {
        let categories = this.state.arrOfCategories.map((el: any, i: number) => {
            ...
            if (el.name === 'all') {
                return (
                    <Link key={i} to={`${PATH.ALL}/All`} className={s.elemNavigation}>
                        {el.name.toUpperCase()}
                    </Link>
                );
            } else if (el.name === 'clothes') {
                return (
                    <Link key={i} to={PATH.CLOTHES} className={s.elemNavigation}>
                        {el.name.toUpperCase()}
                    </Link>
                );
            } else {
                return (
                    <Link key={i} to={PATH.TECH} className={s.elemNavigation}>
                        {el.name.toUpperCase()}
                    </Link>
                );
            }
        });
        return <div className={s.navigation}>{categories}</div>;
    }
}
